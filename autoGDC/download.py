...
Please try investigating the issue - deleting the file and recreating it now...\
\n(File was located at:{self.conf['metadb_path']})",
               exc_info=True)
...
#    print(_metadb.index.name)
#    print(_metadb.index[:10])
#    print(len(all_data_fileids))
#    print(len(_metadb))
#    print(_metadb.index.duplicated().sum())
#    print(_metadb.index.get_level_values(0).duplicated().sum())
#    print(_metadb.loc[~_metadb.index.get_level_values(0).duplicated()])

#    assert len(all_data_fileids) == len(_metadb)
    ...
    # Ensure dtypes are uniform in order to save HDF5 file
    for col, typ in self.conf["metadata_dtypes"].items():
      try:
        metadb[col] = metadb[col].astype(typ)
      except KeyError as e:
        LOG.debug(f"KeyError while changing metadb dtypes: {e}",
                  exc_info = True)

    LOG.debug(f"Metadatabase fully constructed:\n{metadb}\n{metadb.describe()}\n{metadb.dtypes}")

    LOG.info("Saving the new metadata database...")

    metadb.to_hdf(self.conf["metadb_path"],
                  key="metadata",
                  mode="w",
                  data_columns=True,
                  format="table",
...
#      params["size"] = f"{10**9}"
    ...
    LOG.debug(f"GDC search for file_ids matching query: {response}")
...
Please reconsider using `is_manual=True`""")
      ...
      LOG.debug(f"Starting automatic download of study file_ids...")
    ...
    if is_metadata:
      # TODO/FIXME !!
      #   Try using a smaller file_id chunk_size, such that we don't have to
      #   iterate over fields!
      chunk_size = 10000
...
#        sample_metadata_frames += [metadatachunk]
        filt = {"op": "in", "content":{"field":"files.file_id", "value": chunk}}
        ...
        metadata_params = {"format": "json",
                           ...
                           "filters": filt,
                           "size": len(chunk)}
#        self.cache.evict("post_download")
#        self.cache.evict("post_download_metadata")
        o = self._cached_download(url=f"{self.api_url}/files",
                                  post_data=metadata_params,
                                  verbose=True)#,
#                                  expired=True) # REMOVE THIS!
        tmp_df = metadata_json_to_df(
                          o,#BytesIO(o.read()),
                          null_type_strings = self.conf["null_type_strings"])
        sample_metadata_frames += [tmp_df]
    if is_metadata:
      LOG.info("Joining the metadata chunks together...")
      sample_metadata_frames = pd.concat(sample_metadata_frames)
      sample_metadata_frames.to_parquet("metadata.parquet")
      sample_metadata_frames = pd.read_parquet("metadata.parquet")
      frame_bytes = sample_metadata_frames.to_parquet()

      LOG.debug("Storing metadataframe (prior to processing) in download cache")
      self.cache.set("metadb_preprocessed",
                     frame_bytes,
                     read=True,
                     tag="metadata",
  ...
  def _cached_download(self, url,
    ...
    ## POST request
      ...
      if url.endswith("data"):
        tag = "post_download_data"
      elif url.endswith("files"):
        tag = "post_download_metadata"
      else:
        LOG.warn("URL endpoint was not expected. Unclear how to proceed.")
        tag = "post_download"
...
#        urllib.request.urlretrieve(url, target)
        ...
        elif tag == "post_download_data":
          # TODO: How to get tqdm or stream=True for POST request?
          with sess.post(url, data = json.dumps(post_data), headers = post_headers) as r:
            r_head_cd = r.headers.get("Content-Disposition")
            r_head_cd = r.headers.get("content-disposition") if r_head_cd is None else r_head_cd
        ...
        elif tag == "post_download_metadata":
          with sess.post(url, data = json.dumps(post_data), headers = post_headers) as r:
            data = r.json().get("data")
            if data is not None:
              # Get first page
              pages_data = [r.json()]#data.get("hits")
              total_pages = data.get("pagination").get("pages")
              #pages_iter = tqdm(range(1, total_pages), desc="POST Pagination")
              # Get all other pages
              for page in range(1, total_pages):#pages_iter:
                # Modify the params to add "from" (pagination)
                page_params = params
                page_params["from"] = page_chunk_size * page
                with sess.post(url, data=json.dumps(page_params), headers=headers) as page_resp:
                  pages_data += [page_resp.json()]#.get("data").get("hits")

              #fp = path.join(download_dir, file_name)
              with open(target, "w") as output_file:
#                json.dump(flatten_gdc_json_list(pages_data),
#                          output_file,
#                          ensure_ascii=False)
                output_file.write(json.dumps(flatten_gdc_json_list(pages_data)))
  ...
  for d in json_list[1:]:
    for k, v in d.items():
      if k == "data":
        ...
        #   to concat is in d["data"]["hits"]
        for _k, _v in d["data"].items():
          if _k == "hits":
            result["data"]["hits"].update(_v)
          else:
            LOG.warn(f"Unforseen json key {k}")
      elif k == "warnings":
        d.update()
      elif k == "pagination":
        d.update()
